...
export function CardStack<T extends CardStackItem>({
  ...
  // keep active in bounds if items change (e.g. an async-loaded `items` prop
  // resolving after mount) — a render-time clamp can't replace this since the
  // previous length isn't otherwise available to detect the change.
  React.useEffect(() => {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="absolute inset-0">
        {item.imageSrc ? (
          // eslint-disable-next-line @next/next/no-img-element -- generic fallback renderer for arbitrary external image URLs; real usage always supplies a custom renderCard instead.
          <img
            src={item.imageSrc}
            alt={item.title}
